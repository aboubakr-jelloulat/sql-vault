...
(
	SELECT Email, COUNT(*) AS [Duplicate Email] FROM Contacts 
	GROUP BY Email HAVING COUNT(*) > 1
)

SELECT c.ContactID, c.Name, c.Email FROM Contacts c  INNER JOIN  DuplicateEmails dup ON c.Email = dup.Email
...
--ON c.Email = dup.Email
	...
	--Condition for joining: only include rows where the contact’s email exists in the duplicate list.